...
export default function TableMap() {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await apiFetch(`/reservations/${editModal.data.id}`, {
        method: 'PUT',
  ...
  const pickSuggestion = (r) => {
    ...
    setActiveTableId(r.table_name || `Table ${r.table_id}`);
  };
    ...
    <div className="flex flex-col h-full font-sans">
      ...
      <div className="flex gap-4 mb-6">
        <div className="relative w-80">
          <input
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:border-dickens-gold focus:ring-1 focus:ring-dickens-gold"
          />
          {showSuggestions && searchSuggestions.length > 0 && (
            <ul className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-72 overflow-y-auto">
              {searchSuggestions.map(r => (
                <li
                  key={r.id}
                  onMouseDown={(e) => { e.preventDefault(); pickSuggestion(r); }}
                  className="px-4 py-2 cursor-pointer hover:bg-gray-50 border-b last:border-b-0 border-gray-100"
                >
                  <div className="font-medium text-dickens-green text-sm">{r.name}</div>
                  <div className="text-xs text-gray-500">
                    {r.phone || '—'} • {r.date} {r.time?.slice(0, 5)} • {r.table_name || `Bord ${r.table_id}`}
                  </div>
                </li>
              ))}
            </ul>
      ...
      <div className="relative w-full aspect-[4/3] bg-gray-300 rounded-xl border-4 border-gray-400 shadow-inner p-4 max-h-[70vh]">
        ...
        {MAP_TABLES.map((pos) => {
          ...
          const showTooltip = activeTableId === pos.name || hoveredTableId === pos.name;

          return (
            <div
              key={pos.name}
              className={`absolute ${showTooltip ? 'z-[999]' : 'z-10'}`}
                  ...
                  {tableRes.map(r => (
                    <div key={r.id} className="mb-3 last:mb-0">
                      <div className="text-xs text-gray-600 mb-1">
                        ...
                        <div>{formatBookingWindow(r.time)} ({r.guests} pers)</div>
                        <div className="text-gray-400">{r.phone}</div>
                        {r.additionalInfo && (
                          <div className="mt-1 text-gray-700 italic break-words">{r.additionalInfo}</div>
                        )}
                      </div>
